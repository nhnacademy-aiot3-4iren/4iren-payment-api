...
@Component
public class ExpiredTemplate implements MailTemplate<ExpiredMailContext> {
    @Override
    ...
    @Override
    public Optional<String> getMailContent(ExpiredMailContext context) {
        String htmlContent = """
            <div style="font-family: 'Apple SD Gothic Neo', 'Noto Sans KR', sans-serif; max-width: 600px; margin: 0 auto; padding: 40px 20px; background-color: #f9f9f9;">
                <div style="background-color: #ffffff; padding: 40px; border-radius: 12px; box-shadow: 0 4px 6px rgba(0,0,0,0.05); text-align: center;">
                    <h1 style="color: #333333; font-size: 24px; margin-bottom: 20px;">결제 실패로 인한 구독 만료 안내</h1>
                    <p style="color: #666666; font-size: 16px; line-height: 1.6; margin-bottom: 30px;">
                        안녕하세요.<br>결제가 반복적으로 실패하여 <strong>%s</strong> 플랜 구독이 만료되었습니다.
                    </p>
                    <div style="background-color: #f0f4f8; border: 1px solid #dce4ec; border-radius: 8px; padding: 20px; margin-bottom: 30px; text-align: left;">
                        <p style="margin: 0 0 10px 0; color: #333333; font-size: 15px;">만료 일시&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;%s</p>
            ...
            """.formatted(context.plan(), context.expiredAt(), context.failureReason());
